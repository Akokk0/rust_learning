...
pub async fn get_all_teachers_db(pool: &PgPool) -> Result<Vec<Teacher>, MyError> {
    ...
    /*let rows = sqlx::query(
        &format!("SELECT id, name, picture_url, profile FROM teacher")
    )
        .fetch_all(pool)
        .await?;*/
...
pub async fn get_teacher_details_db(pool: &PgPool, teacher_id: i32) -> Result<Teacher, MyError> {
    let row = sqlx::query!(
        ...
        .map_err(|_err| MyError::NotFound("Teacher id is not found".into()))?;
...
pub async fn update_teacher_details_db(
    ...
    /*let update_row = sqlx::query(
        &format!("UPDATE teacher SET name = {}, picture_url = {}, profile = {} WHERE id = {}\
        RETURNING id, name, picture_url, profile",
                 temp.name,
                 temp.picture_url,
                 temp.profile,
