...
pub async fn get_courses_for_teacher(
    app_state: web::Data<AppState>,
    params: web::Path<i32>
    // web::Path(teacher_id): web::Path<i32> // 可以直接把teacher_id抽取出来 57行可以省略
) -> Result<HttpResponse, MyError> {
    ...
    if filtered_courses.len() > 0 {
        ...
        HttpResponse::Ok().json("No courses found for teacher")
    }*/

    // let teacher_id: i32 = i32::try_from(params.into_inner()).unwrap();
...
pub async fn get_course_detail(
    ...
    // web::Path((teacher_id, course_id)): web::Path<(i32, i32)> // 85, 86, 87行可省略
) -> Result<HttpResponse, MyError> {
    ...
    let (teacher_id, course_id) = params.into_inner();
    // let teacher_id: i32 = i32::try_from(teacher_id).unwrap();
...
pub async fn update_course_details(
    ...
    update_course_details_db(
        &app_state.db, teacher_id, course_id, update_course.into()
    )
        .await
        .map(|resp| HttpResponse::Ok().json(resp))
}
...
mod tests {
    ...
    #[actix_rt::test]
    async fn update_course_failure() {
        dotenv().ok();
